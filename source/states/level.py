...
from ..commponents import player, stuff, brick, box, enemy
import os
import json
...
class Level:
    def start(self, game_info):
        ...
        self.finished = False
        self.next = C.LAST
        self.info = info.Info(C.LEVEL, self.game_info)
        self.load_map_data()
        ...
        self.overhead_info_display = info.Info(C.LEVEL, self.game_info)
    ...
    def setup_background(self):
        ...
        self.backgrond = setup.GRAPHICS[self.image_name]
        rect = self.backgrond.get_rect()
        self.backgrond = pygame.transform.scale(self.backgrond, (int(rect.width * C.BG_MULTI),
                                                                 int(rect.height * C.BG_MULTI)))
        self.backgrond_rect = self.backgrond.get_rect()
        self.game_window = setup.SCREEN.get_rect()
        self.game_ground = pygame.Surface((self.backgrond_rect.width, self.backgrond_rect.height))
    ...
    def update(self, surface, keys):
        ...
        elif self.player.rect.centerx >14450:
            self.finished = True
            self.next = 'juqing2'
    ...
    def play_death_song(self):
        ...
        if self.player.death_timer == 0:
            self.player.death_timer = self.curren_time
        elif (self.curren_time - self.player.death_timer) > 3000:
            self.update_game_info()
    def is_frozen(self):
    ...
    def check_x_collisions(self):
        ...
        if enemy and self.player.state != C.ATTACK:
            if self.player.big:
                ...
                self.player.x_vel = self.player.x_vel * -0.8
                self.player.hurt_immune = True
                if self.player.life == 0:
                    self.player.go_die(self.game_info)
        elif self.player.state == C.ATTACK and enemy:
                self.player.hurt_immune = False
                enemy.go_die('trampled')
    ...
    def draw(self, surface):
        self.game_ground.blit(self.backgrond, self.game_window, self.game_window)
